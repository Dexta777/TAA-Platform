-- Persist canonical discount snapshots and redemptions during atomic paid checkout finalization.
...
BEGIN
  ...
  IF FOUND THEN
    ...
    SELECT
      v_checkout_intent.discount_code_id,
      v_checkout_intent.id,
      orders.id,
      orders.user_id,
      v_checkout_intent.discount_code,
      orders.customer_email_fingerprint,
      orders.shipping_phone_fingerprint,
      orders.shipping_address_fingerprint,
      v_checkout_intent.discount_amount,
      v_checkout_intent.shipping_discount_amount
    FROM public.orders
    WHERE orders.id = v_order_id
      AND v_checkout_intent.discount_code_id IS NOT NULL
      AND NOT EXISTS (
        SELECT 1
        FROM public.discount_redemptions
        WHERE discount_redemptions.checkout_intent_id = v_checkout_intent.id
          OR discount_redemptions.order_id = orders.id
      );
